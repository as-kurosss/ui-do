...
import { insertNode as treeInsert, removeNode as treeRemove, moveNode as treeMove, mapNode, createDefaultRoot, findParent, findNode, wrapNode as treeWrap, ungroupChildren as treeUngroup } from '../core/tree'
    ...
    (set, get) => ({
      ...
      duplicateNode: (nodeId) => {
        set((state) => {
          ...
          const newRoot = treeInsert(screen.root, parentInfo.parent.id, parentInfo.index + 1, clone)
          return updateScreenRoot(state, newRoot)
        })
      },
      ...
      removeScreen: (id) => {
        set((state) => {
          ...
          const newActive = state.activeScreenId === id
            ? (filtered[0]?.id ?? '')
...
function assignNewIds(node: SpecNode): void {
  node.id = generateId()
  const children =
    node.kind === 'layout' ? node.children :
