...
function ContainerNode({ node, selectedId, onSelect, onContextMenu }: {
...
function LayoutNodeContent({ node, isSelected, onSelect, onContextMenu, droppableRef }: {
  node: LayoutNode
  isSelected: boolean
  onSelect: (id: NodeId) => void
  onContextMenu?: (id: NodeId, e: React.MouseEvent) => void
  ...
  return (
    ...
    >
      {node.children.map((child) => (
        <CanvasNode key={child.id} node={child} selectedId={null} onSelect={onSelect} onContextMenu={onContextMenu} />
      ))}
...
function ComponentNodeContent({ node, isSelected, onSelect, onContextMenu, droppableRef }: {
  node: ComponentNode
  isSelected: boolean
  onSelect: (id: NodeId) => void
  onContextMenu?: (id: NodeId, e: React.MouseEvent) => void
  ...
  if (!Comp) {
    const borderRadius = getComputedStyle(document.body).getPropertyValue('--radius').trim() || '0.5rem'
    return (
      <div
        ref={droppableRef}
        data-bn-id={node.id}
  ...
  return (
    <>
      ...
      <div onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); onContextMenu?.(node.id, e) }}>
...
function TextNodeContent({ node, onSelect, onContextMenu }: {
  node: TextNode
  onSelect: (id: NodeId) => void
  onContextMenu?: (id: NodeId, e: React.MouseEvent) => void
}) {
  ...
  return (
    <Tag
      ...
      onContextMenu={(e: React.MouseEvent) => { e.preventDefault(); e.stopPropagation(); onContextMenu?.(node.id, e) }}
    >
      {node.text}
    </Tag>
  )
...
function CodeNodeContent({ node, onSelect, onContextMenu }: {
  node: CodeNode
  onSelect: (id: NodeId) => void
  onContextMenu?: (id: NodeId, e: React.MouseEvent) => void
}) {
...
function CanvasNode({ node, selectedId, onSelect, onContextMenu }: {
  node: SpecNode
  selectedId: NodeId | null
  onSelect: (id: NodeId) => void
  onContextMenu?: (id: NodeId, e: React.MouseEvent) => void
  ...
  switch (node.kind) {
    ...
    case 'component':
      return (
        <ContainerNode node={node} selectedId={selectedId} onSelect={onSelect} onContextMenu={onContextMenu} />
      )
    case 'text':
...
export function CanvasRenderer({ node, selectedId, onSelect, onContextMenu }: CanvasRendererProps) {
  return (
    <CanvasNode node={node} selectedId={selectedId} onSelect={onSelect} onContextMenu={onContextMenu} />
  )
}
